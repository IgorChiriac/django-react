...
export default function LoginPage() {
    ...
    const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        AuthenticationService.login(username, password)
            ...
            .catch((error) => {
                if (error.response) {
                    setLoginState({ loading: false, error: error.response?.data?.detail });
                }
            });
    };
    ...
    return (
        <ThemeProvider theme={theme}>
            <Container component="main" maxWidth="xs">
                ...
                >
                    {loginState.error && <Alert severity="error">{loginState.error}</Alert>}
                    ...
                    <Box component="form" onSubmit={handleSubmit} sx={{ mt: 1 }}>
                        ...
                        <Button type="submit" disabled={loginState.loading} fullWidth variant="contained" sx={{ mt: 3, mb: 2 }}>
                            Sign In!
                        </Button>
                        <Grid container>
                            <Grid item>
